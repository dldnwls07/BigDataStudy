...
# 분석에 사용할 모든 컬럼
analysis_columns = list(column_mapping.values())  # 중복 제거

# 각 변수와 시험 점수 간의 관계 시각화
for column in analysis_columns[:-1]:  # '시험 점수' 제외
    plt.figure(figsize=(8, 6))
    if data[column].dtype == 'object':  # 범주형 데이터
        data.groupby(column)['시험 점수'].mean().plot(kind='bar', color='skyblue')
        plt.title(f'{column}에 따른 시험 점수 평균')
        plt.ylabel('시험 점수 평균')
        plt.xlabel(column)
    else:  # 연속형 데이터
        plt.scatter(data[column], data['시험 점수'], alpha=0.5, color='skyblue')
        plt.title(f'{column}와 시험 점수의 관계')
        plt.ylabel('시험 점수')
        plt.xlabel(column)
