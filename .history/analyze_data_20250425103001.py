...
data_encoded['동기 수준'] = data['동기 수준'].map(motivation_map)
...
data_encoded['교사의 질'] = data['교사의 질'].map(teacher_map)
...
stress_factors = ['수면 시간', '공부한 시간', '동기 수준', '학습 장애 여부']
print("\n시험 성적과 스트레스 간접 요인 상관 분석:")
print(data_encoded[stress_factors + ['시험 점수']].corr())

# 그룹별 비교 (인코딩 전 원본 데이터 사용)
# 동기 수준이 낮을수록 성적이 낮은지 확인
low_motivation = data[data['동기 수준'] == 'Low']
medium_motivation = data[data['동기 수준'] == 'Medium']
high_motivation = data[data['동기 수준'] == 'High']
...
sns.boxplot(x='동기 수준', y='시험 점수', data=data, order=['Low', 'Medium', 'High'])
plt.title('동기 수준별 시험 점수')
plt.xlabel('동기 수준')
plt.ylabel('시험 점수')
plt.savefig('동기수준_시험점수_박스플롯.png')
plt.show()

# 수면 시간과 시험 점수 산점도 (색상을 동기 수준으로 구분)
plt.figure(figsize=(10, 6))
for level in ['Low', 'Medium', 'High']:
    subset = data[data['동기 수준'] == level]
    plt.scatter(subset['수면 시간'], subset['시험 점수'], 
                alpha=0.7, label=f'동기 수준: {level}')
plt.title('수면 시간과 시험 점수의 관계 (동기 수준별)')
plt.xlabel('수면 시간')
plt.ylabel('시험 점수')
plt.legend()
...
plt.figure(figsize=(10, 6))
sns.scatterplot(x='공부한 시간', y='시험 점수', hue='동기 수준', 
                data=data, palette='viridis', s=100, alpha=0.7)
plt.title('공부한 시간과 시험 점수의 관계 (동기 수준별)')
plt.xlabel('공부한 시간')
plt.ylabel('시험 점수')
plt.savefig('공부시간_시험점수_동기수준_산점도.png')
plt.show()
...
if '진로 유형' in data.columns:
    # 데이터 전처리
    features = data[['공부한 시간', '동기 수준', '이전 시험 성적']]  # 주요 특징 선택
    labels = data['진로 유형'] 
