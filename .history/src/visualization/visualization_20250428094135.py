...
def plot_score_distribution(train_csv, test_csv, output_path):
    # 학습 데이터와 테스트 데이터 로드
    ...
    # 시험 점수 분포 시각화
    plt.figure(figsize=(10, 6))
    plt.hist(train_data['Exam_Score'], bins=30, alpha=0.5, label='Train Data', color='blue')
    plt.hist(test_data['Exam_Score'], bins=30, alpha=0.5, label='Test Data', color='orange')
    plt.title('Distribution of Exam Scores in Train and Test Data')
...
if __name__ == "__main__":
    ...
    output_path = "output/picture/train_results/score_distribution_comparison.png"
    plot_score_distribution(train_csv, test_csv, output_path)